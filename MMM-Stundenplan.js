Module.register("MMM-Stundenplan", {
  ...
  // Ab 12 Uhr nächsten Tag anzeigen
  const jetzt = moment();
  ...
  for (let d = 0; d < this.config.daysToShow; d++) {
    ...
    if (alleZeiten.length === 0) {
      ...
      alleZeiten.forEach(zeit => {
        ...
        this.config.kinder.forEach(k => {
          ...
          td.style.backgroundColor = k.bgcolor;
          td.style.color = k.color;

          const stunde = (k.plan[tagName] || []).find(s => s.zeit === zeit);
          if (stunde) {
            td.innerHTML = `<div class="fach">${stunde.fach} <span class="raum">${stunde.raum || ""}</span></div>` ;
          }
          tr.appendChild(td);
        });
